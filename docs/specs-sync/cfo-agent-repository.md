# cfo-agent Repository Specification
...
`cfo-agent` is a **deployed Python package** that provides `CFOAgent` — the Chief Financial Officer agent of the Agent Operating System. `CFOAgent` extends `LeadershipAgent` with financial strategy, budgeting, fiscal oversight, and boardroom orchestration capabilities. It is deployed as an Azure Functions app and registered with the Foundry Agent Service.
...
| Concern | Owner |
|---------|-------|
| Financial leadership agent (`CFOAgent`) | **cfo-agent** |
| Finance-domain boardroom participation | **cfo-agent** |
...
`cfo-agent` is **deployed** — it runs as an Azure Functions app registered with the Foundry Agent Service as the financial specialist in boardroom orchestrations.
...
│       └── agent.py             # CFOAgent — finance and leadership dual-purpose
├── tests/
│   ├── conftest.py              # Shared pytest fixtures
│   └── test_cfo_agent.py        # pytest unit tests
├── docs/
│   ├── api-reference.md
│   └── contributing.md
...
agent = CFOAgent(
    agent_id="cfo",
    purpose="Financial strategy, budgeting, and fiscal oversight of the organisation",
...
`CFOAgent` uses the `finance` LoRA adapter, enabling domain-specific financial reasoning:

```python
CFOAgent(adapter_name="finance")  # → finance domain LoRA adapter
```
...
# With coverage
pytest tests/ --cov=cfo_agent --cov-report=term-missing --rootdir=.

# Lint
pylint src/cfo_agent

# Type check
mypy src/cfo_agent

# Specific test
pytest tests/test_cfo_agent.py -v -k "test_initialize" --rootdir=.
```
...
2. **Finance-first** — Decisions are evaluated through a financial strategy and fiscal responsibility lens
3. **Boardroom specialist** — Participates as the financial domain expert in CEO-coordinated boardroom sessions
